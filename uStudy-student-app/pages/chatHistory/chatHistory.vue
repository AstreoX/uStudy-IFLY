      ...
      <template v-if="!isSearching">
        <view class="nav-main">
          ...
          <view class="nav-title-wrap">
            <text class="nav-title-main">对话历史</text>
...
export default {
  ...
  onShow() {
    ...
    if (this.spaceId && !this.isLoading) {
      this.loadConversations()
    }
  },
  ...
  methods: {
    ...
    createNewConversation() {
      uni.navigateTo({
        url: `/pages/spaceChat/spaceChat?id=${this.spaceId}&name=${encodeURIComponent(this.spaceName)}`
      })
    },

    openConversation(conv) {
      uni.navigateTo({
        url: `/pages/spaceChat/spaceChat?id=${this.spaceId}&name=${encodeURIComponent(this.spaceName)}&conversationId=${conv.id}`
      })
    },
    ...
    async doSearch() {
      ...
      try {
        const result = await searchSpaceConversations(this.spaceId, query)
    ...
    async doDeleteConversation() {
      ...
      try {
        ...
        this.conversations = this.conversations.filter(c => c.id !== this.selectedConvId)
