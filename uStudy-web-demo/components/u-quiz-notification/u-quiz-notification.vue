...
export default {
  ...
  computed: {
    ...
    titleText() {
      if (this.isFailed) return '评估失败'
      return `${this.score}/${this.totalScore} 分`
    },
    subText() {
      const topic = this.quizTopic || '测验'
      if (this.isFailed) return `${topic} - 点击重试`
      return `${topic} - 点击查看详情`
    },
    positionStyle() {
      return {
...
.quiz-notification {
  ...
  border: 1px solid rgba(73, 255, 170, 0.25);
...
.quiz-notification.notification-failed {
  border-color: rgba(255, 50, 66, 0.3);
}

.quiz-notification.notification-enter {
  opacity: 1;
...
.notification-icon-wrap {
  ...
  background: rgba(73, 255, 170, 0.15);
}
