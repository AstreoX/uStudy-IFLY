...
assert(sidebar.includes('getTeacherSpaces(this.spaces)'), 'teacher navigation is not role-gated through the shared owner/teacher policy')
...
assert(teacherDashboard.includes('dayOptions: [7, 30, 90, 365]'), 'teacher dashboard is missing the one-year range')
assert(teacherDashboard.includes("days === 365 ? '近 1 年'"), 'teacher dashboard one-year range label is missing')
...
assert(teacherSpaceSelection.includes("new Set(['owner', 'teacher'])"), 'teacher course policy must accept owners and teacher members')
assert(teacherSpaceSelection.includes('spaces.filter(isTeacherSpace)'), 'teacher course filtering must use the shared role policy')
...
assert(studyPage.includes("currentSpace?.is_managed_course === true"), 'study page does not consume the managed-course response flag')
assert(studyPage.includes('v-if="canDeleteSpace"'), 'managed courses do not hide destructive space actions')
assert(studyPage.includes('canRenameSpace()'), 'managed-course rename capability is not represented')
assert(studyPage.includes("this.userRole === 'owner'"), 'course owners cannot edit the managed course graph')
assert(studyPage.includes('if (this.isManagedCourse) return false'), 'managed-course members can still enter graph editing')
assert(studyPage.includes('isCollaborative && canInspectMemberGraphs'), 'student graph member selector is not role-gated')
assert(studyPage.includes(':targetUserId="graphTargetUserId"'), 'student graph requests can still send a foreign user id')
assert(studyPage.includes('v-if="!isManagedCourse" class="manage-toggle-row"'), 'managed-course graph edit delegation is still visible')
const notesPanel = read('components/study/notes/NotesPanel.vue')
assert(notesPanel.includes("note?.visibility === 'private'"), 'teacher UI still offers edits for private student notes')
