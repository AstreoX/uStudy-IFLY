...
以下是根据用户消息从学习资料中检索到的相关内容片段，每个片段有编号标记：

{rag_context}

**引用规则（必须遵守）**：
- 你**必须**在回答中积极使用方括号标注来源编号。只要你的回答内容与上述任何文档片段相关或受其启发，就应在相关句子末尾标注 [1]、[2] 等编号
- 多个来源可连续标注，如 [1][3]
- 每个引用编号对应上面的片段编号，不要编造不存在的编号
- 即使你对内容进行了改写、总结或综合，只要信息来源于上述片段，仍然需要标注
- 目标：让用户清楚知道你的每一个关键论述来自哪份资料。宁可多标注，不要遗漏
